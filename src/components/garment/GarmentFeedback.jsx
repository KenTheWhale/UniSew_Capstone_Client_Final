...
const FeedbackCard = React.memo(({feedback, onImageClick, onViewDetail, onAppeal, onViewAppeals}) => {
    const isReport = feedback.report;
    
    // Debug logging để kiểm tra appeals data
    const shouldShowAppeals = feedback.report && (
        (feedback.appeals && feedback.appeals.length > 0) ||
        (feedback.appeals && Array.isArray(feedback.appeals)) ||
        feedback.id
    );
    
    console.log('FeedbackCard appeals debug:', {
        id: feedback.id,
        isReport,
        report: feedback.report, // Check both fields
        appeals: feedback.appeals,
        appealsLength: feedback.appeals?.length,
        appealsIsArray: Array.isArray(feedback.appeals),
        shouldShowAppeals,
...
export default function GarmentFeedback() {
    ...
    // Appeals management handlers
    const handleViewAppeals = useCallback((feedback) => {
        setSelectedFeedback(feedback);
        
        // Use existing appeals or create mock data for testing
        const mockAppeals = feedback.appeals || [
            {
                id: 1,
                reason: "The quality assessment was unfair. The fabric defects mentioned in the report are minimal and within industry standards. We have photographic evidence showing that the uniforms meet the agreed specifications.",
            ...
            {
                id: 2,
                reason: "The delivery delay was due to force majeure circumstances beyond our control. We provided alternative solutions and partial compensation, which should be considered in the final decision.",
                videoUrl: null,
                status: "under-review",
        ...
        setSelectedAppeals(mockAppeals);
        setAppealsDialogOpen(true);
    }, []);

    const handleCloseAppeals = useCallback(() => {
        setAppealsDialogOpen(false);
        setSelectedAppeals([]);
        setSelectedFeedback(null);
    }, []);

    const handleApproveAppeal = useCallback(async (appealId) => {
        try {
            setApprovingAppeal(true);
            
            // TODO: Replace with actual API call
            // const response = await approveAppeal(appealId);
            
            // Simulate API call for now
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            enqueueSnackbar('Appeal approved successfully', {variant: 'success'});
            ...
            enqueueSnackbar('Failed to approve appeal', {variant: 'error'});
        } finally {
            setApprovingAppeal(false);
        }
    }, [fetchFeedbacks, handleCloseAppeals]);
                        ...
                        <Box sx={{display: 'flex', flexDirection: 'column', gap: 3}}>
                            ...
                            <Paper elevation={0} sx={{
                                ...
                                <Typography variant="h6" sx={{fontWeight: 'bold', mb: 3, color: '#1e293b'}}>
                                    Appeals Submitted ({selectedAppeals.length})
                                </Typography>
                                ...
                                {selectedAppeals.length === 0 ? (
                                    <Box sx={{
                                        ...
                                        <Typography variant="body2" sx={{color: '#94a3b8'}}>
                                            No appeals have been submitted for this report yet.
                                        </Typography>
                                    </Box>
                                ) : (
                                    <Box sx={{display: 'flex', flexDirection: 'column', gap: 2}}>
                                        {selectedAppeals.map((appeal, index) => (
                                            <Card key={appeal.id || index} elevation={0} sx={{
                                                border: '1px solid #e2e8f0',
                                                borderRadius: 2,
                                                transition: 'all 0.3s ease',
                                                '&:hover': {
                                                ...
                                                <CardContent sx={{p: 3}}>
                                                    <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2}}>
                                                        <Box sx={{flex: 1}}>
                                                            <Box sx={{display: 'flex', alignItems: 'center', gap: 2, mb: 1}}>
                                                                ...
                                                                <Box>
                                                                    ...
                                                                    <Typography variant="body2" sx={{color: '#64748b'}}>
                                                                        {appeal.creationDate ? formatDate(appeal.creationDate) : 'Recently submitted'}
                                                                    </Typography>
                                                                </Box>
                                                            </Box>
                                                            ...
                                                            <Box sx={{mb: 2}}>
                                                                <Chip
                                                                    label={appeal.status || 'Pending Review'}
                                                                    sx={{
                                                                        backgroundColor: (() => {
                                                                            const status = appeal.status?.toLowerCase() || 'pending';
                                                                            if (status.includes('approved') || status.includes('accepted')) return '#dcfce7';
                                                                            if (status.includes('rejected')) return '#fecaca';
                                                                            return '#fef3c7';
                                                                        })(),
                                                        ...
                                                        <Box sx={{display: 'flex', flexDirection: 'column', gap: 1, ml: 2}}>
                                                            {(!appeal.status || appeal.status.toLowerCase().includes('pending') || appeal.status.toLowerCase().includes('review')) && (
                                                                <Button
                                                                    variant="contained"
                                                                    size="small"
                                                                ...
                                                                >
                                                                    {approvingAppeal ? 'Processing...' : 'Approve'}
                                                                </Button>
                                                            )}
