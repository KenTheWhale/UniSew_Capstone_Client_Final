...
export default function DesignerTransaction() {
    ...
    const data = mapToCsvRows(transactions, designerCsvData);
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            align: 'right',
            width: 180,
            sorter: (a, b) => ((a?.remain && (a.receiver?.account?.role === 'designer' ? a.remain.receiver : a.remain.sender)) || 0) - ((b?.remain && (b.receiver?.account?.role === 'designer' ? b.remain.receiver : b.remain.sender)) || 0),
            render: (_, record) => {
                const isReceiver = record?.receiver?.account?.role === 'designer';
                    ...
                    <Box sx={{display: "flex", alignItems: "center", gap: 2}}>
                        <Tooltip title="Download PDF Report">
                            <Button
                                ...
                                onClick={() => handleDownloadPdf(transactions, designerPDFHeader, designerPDFBody(transactions))}
                                sx={{
